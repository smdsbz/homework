...
parser.add_argument('-a', '--append-encoding', action='store_true',
                    help='append encoding to new files\' names')

## change line endings
parser.add_argument('-no-crlf', '--dont-convert-lf-to-crlf',
                    action='store_true',
...
for filename in filenames:
    ...
    with open(filename, 'rt', encoding=args['infile_encoding']) as f:
        ...
        # create new file
        old_name, extension = os.path.splitext(os.path.basename(filename))
        appendix = ''
        if args['append_encoding']:
            appendix = '_' + args['outfile_encoding']
        # TODO: change #include filenames as well
        new_name = '{original}{appendix}{extension}'.format(
                original=old_name,
                appendix=appendix,
                extension=extension
        )
